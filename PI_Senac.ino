...
void loop()                     // executar uma e outra vez
{
    ...
    // Frequência do sensor (Hz) = 7.5 * Q (Litros/min)
    // Liters = Q * tempo decorrido (segundos) / 60 (segundos/minuto)
    // Liters = (Frequencia (Pulses/segundo) / 7.5) * tempo decorrido (segundos) / 60
    ...
    // Print para aferir os sensores no arduino
    // Serial.print (liters);
